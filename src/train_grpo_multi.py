"""
Train a language-specific GRPO joke generation model with multi-GPU support.

Usage (multi-GPU with accelerate):
    accelerate launch --num_processes 4 train_grpo_multi.py --language en --model_id Qwen/Qwen2.5-7B-Instruct
    accelerate launch --num_processes 4 train_grpo_multi.py --language zh --model_id Qwen/Qwen2.5-7B-Instruct
    accelerate launch --num_processes 4 train_grpo_multi.py --language es --model_id Qwen/Qwen2.5-7B-Instruct
"""
import os
import torch
...
class GenerationLoggingCallback(TrainerCallback):
    ...
    Only logs on the main process in distributed training.
    """
    ...
    def on_step_end(self, args, state, control, model=None, **kwargs):
        ...
        # Get the actual model (unwrap if using FSDP/DDP)
        ...
        with torch.no_grad():
            for prompt in prompts:
                inputs = self.tokenizer(prompt, return_tensors="pt").to(unwrapped_model.device)
...
def main():
    """Main function to set up and run multi-GPU GRPO training for joke generation."""
    args = parse_args()

    # Load environment variables (e.g., WANDB_API_KEY, HF_TOKEN)
    load_dotenv()

    # Initialize accelerator to get process info
    accelerator = Accelerator()
    is_main_process = accelerator.is_main_process

    # Helper for printing only on main process
    def main_print(*msg):
        if is_main_process:
            print(*msg)

    main_print(f"Loading datasets from {args.train_data_file} and {args.test_data_file}...")
    main_print(f"Training language: {args.language}")
    ...
    try:
        ...
        main_print(f"Error loading datasets. Ensure the paths are correct. Error: {e}")
        return

    main_print(f"Using device: {accelerator.device}")
    main_print(f"Distributed type: {accelerator.distributed_type}")

    # For multi-GPU training, don't use device_map="auto"
    # Let the Trainer/accelerate handle device placement
    model = AutoModelForCausalLM.from_pretrained(
        args.model_id,
        torch_dtype=torch.bfloat16,
        trust_remote_code=True,
        # Don't use device_map for multi-GPU - accelerate handles this
    )
    ...
    # Note: These will be called on each process, so each process loads its own reward model
    lang = args.language
    ...
    main_print(f"Using joke rater model: {args.joke_rater_model}")
    main_print(f"All reward functions configured for language: {lang}")
    ...
    # Configure GRPO Training for multi-GPU
    ...
    main_print(f"Starting multi-GPU GRPO training for {lang_suffix} jokes...")
    trainer.train()
    ...
    # Wait for all processes before demo
    accelerator.wait_for_everyone()

    # Run generation demo (only on main process)
    if is_main_process:
        ...
        inputs = tokenizer(prompt, return_tensors="pt").to(unwrapped_model.device)
