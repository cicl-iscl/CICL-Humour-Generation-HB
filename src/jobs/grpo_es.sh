...
echo "Starting Spanish GRPO training on 1 GPU..."

python3 train_grpo.py \
    --model_id "Qwen/Qwen2.5-7B-Instruct" \
    --language "es" \
    --output_dir "./checkpoints/grpo-es" \
...
# Check exit status
if [ $? -eq 0 ]; then
    echo "Spanish GRPO training completed successfully."
else
    echo "Spanish GRPO training failed with exit code $?."
fi
