...
SCRIPT_ARGS=" \
    ...
    --language en \
    --output_dir ./checkpoints/grpo-en-multi \
...
echo "Starting English GRPO training on $GPUS_PER_NODE GPUs with accelerate..."

accelerate launch --num_processes $GPUS_PER_NODE $SCRIPT $SCRIPT_ARGS

# Check exit status
if [ $? -eq 0 ]; then
    echo "English GRPO multi-GPU training completed successfully."
else
    echo "English GRPO multi-GPU training failed with exit code $?."
fi
