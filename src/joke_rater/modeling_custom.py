...
class HierarchicalDebertaConfig(DebertaV2Config):
    """Config for mDeBERTa-based hierarchical classifier."""
    model_type = "mdeberta-joke-rater"
...
class HierarchicalDebertaClassifier(DebertaV2PreTrainedModel):
    """
    A custom mDeBERTa model with a hierarchical classification head (binary
    and child) and a combined loss function including classification and regression.
    """
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, labels=None):
        # mDeBERTa uses token_type_ids, pass them if provided
        ...
        log_probs_nonzero = log_pb[:, 1].unsqueeze(-1) + log_pc
        logits = torch.cat(
            [log_pb[:, 0].unsqueeze(-1), log_probs_nonzero], dim=-1
        )
